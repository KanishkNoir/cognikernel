...
from cognikernel.config import VALID_HOOK_POLICIES, Config
...
def test_project_overlay_layered_on_global(tmp_path: Path, monkeypatch) -> None:
    ...
    # Global ~/.cognikernel/config.toml sets one field.
    global_dir = tmp_path / "userhome" / ".cognikernel"
...
def test_cognikernel_dir_env_var_preserved_when_project_overlay_applies(
    tmp_path: Path, monkeypatch,
) -> None:
    """The COGNIKERNEL_DIR env override survives project overlay (key fix for hooks)."""
    custom_dir = tmp_path / "custom_cognikernel"
    monkeypatch.setenv("COGNIKERNEL_DIR", str(custom_dir))
    ...
    cfg = Config.load(project_path=project)
    assert cfg.cognikernel_dir == custom_dir
    assert cfg.hook_policy == "strict"      # both env and project apply


def test_project_config_can_override_cognikernel_dir(
    tmp_path: Path, monkeypatch,
) -> None:
    """Project config explicitly setting cognikernel_dir wins over env."""
    monkeypatch.setenv("COGNIKERNEL_DIR", str(tmp_path / "env_dir"))

    project = tmp_path / "myproj"
    custom = tmp_path / "project_specific"
    (project / ".cognikernel").mkdir(parents=True)
    (project / ".cognikernel" / "config.toml").write_text(
        f'cognikernel_dir = "{custom.as_posix()}"\n',
        encoding="utf-8",
    )

    cfg = Config.load(project_path=project)
    assert cfg.cognikernel_dir == Path(custom.as_posix())
