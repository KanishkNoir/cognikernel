...
@dataclass
class Config:
    cognikernel_dir: Path = field(default_factory=lambda: Path.home() / ".cognikernel")
    ...
    # unchanged until they opt in. The `COGNIKERNEL_EXTRACTOR` env var, when set,
    ...
    # Claude's SessionStart (and `cognikernel codex-sync`) scans ~/.codex/sessions for
    ...
    # checkout's .cognikernel/config.toml, all clients resolve to the same project DB.
    ...
    def projects_dir(self) -> Path:
        return self.cognikernel_dir / "projects"

    @property
    def logs_dir(self) -> Path:
        return self.cognikernel_dir / "logs"
    ...
    def load_with_issues(
        ...
        *,
        ...
        Precedence (highest first):
          1. `<project_path>/.cognikernel/config.toml`  (when project_path is given)
          2. `~/.cognikernel/config.toml`  (the global file, or `config_path` if provided)
          3. Built-in defaults
        ...
        The COGNIKERNEL_DIR env var short-circuits everything for test/CI use.
        ...
        """
        issues: list[str] = []
        # COGNIKERNEL_DIR env var lets tests (and CI) redirect the data directory
        # without touching ~/.cognikernel. It overrides cognikernel_dir specifically,
        # but project-local overlays still apply on top of it.
        env_dir = os.environ.get("COGNIKERNEL_DIR")

        if config_path is None:
            config_path = Path.home() / ".cognikernel" / "config.toml"

        # Layer 2 — global config.
        if env_dir:
            base = cls(cognikernel_dir=Path(env_dir))
        ...
        if project_path is not None:
            project_cfg_path = Path(project_path) / ".cognikernel" / "config.toml"
            if project_cfg_path.exists():
                project_kwargs = cls._read_toml_kwargs(project_cfg_path, issues)
                # Replace only the fields the project file specifies.
                # cognikernel_dir override from COGNIKERNEL_DIR is preserved unless the
                # project config explicitly sets a different cognikernel_dir.
