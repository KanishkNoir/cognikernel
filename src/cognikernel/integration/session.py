...
def session_end(
    ...
    from cognikernel.storage.jobs import ack_stage, enqueue_extraction, fail_job, recover_stuck_running_jobs
...
def session_capture(
    ...
    """
    from cognikernel.storage.cursors import get_cursor, slice_storage_delta
    from cognikernel.storage.evidence import store_evidence
    from cognikernel.storage.jobs import enqueue_extraction, recover_stuck_running_jobs
...
def _worker_log(project_id: str, msg: str) -> None:
    ...
    try:
        log_dir = Path.home() / ".cognikernel" / "logs"
...
def _acquire_worker_lock(
    project_id: str,
    cognikernel_dir: Path | None = None,
    ...
    The lock lives under `cognikernel_dir` — the same root as the project DB — so it
    honours COGNIKERNEL_DIR / per-project config and works under a read-only home
    (audit P2). A hardcoded ~/.cognikernel split-brained workers whose data dir was
    relocated and broke entirely under a read-only home. Defaults to ~/.cognikernel
    ...
    """
    import os
    base = cognikernel_dir if cognikernel_dir is not None else (Path.home() / ".cognikernel")
    lock_dir = base / "locks"
...
def process_jobs(
    ...
    subprocess spawned by `cognikernel capture`. Processes jobs in oldest-first order,
    ...
    from cognikernel.storage.cursors import get_cursor, save_cursor, slice_jsonl_for_extraction
    ...
    lock_path = _acquire_worker_lock(project_id, cognikernel_dir=config.cognikernel_dir)
...
def replay_job(
    ...
    """
    from cognikernel.storage.evidence import load_evidence, load_full_transcript
    from cognikernel.storage.jobs import get_job, replay_dead_letter
