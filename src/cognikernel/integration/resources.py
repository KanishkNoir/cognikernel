...
def render_skeleton(
    ...
    try:
        with get_connection(db_path) as conn:
            run_migrations(conn)
            from cognikernel.symbols.store import load_symbol_edges, load_symbol_nodes
...
def render_state(project_id: str, config: Config | None = None) -> str:
    ...
    try:
        ...
        project_path = row["value"] if row and row["value"] else None
        if not project_path:
            return "Project path not yet recorded. Run `cognikernel init <path>` to register."
        from cognikernel.integration.session import render_state as _render
