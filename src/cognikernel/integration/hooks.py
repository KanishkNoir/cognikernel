...
invoked two ways with *identical* behavior:
  - ``python -m cognikernel hook-<event>`` — path-portable; what ``cognikernel init`` writes.
  - the thin ``scripts/cognikernel_*_hook.py`` shims — back-compat for older settings.json.
...
_log = logging.getLogger("cognikernel.hooks")
...
def posttool_grep_main() -> None:
    ...
    try:
        ...
        from cognikernel.storage.grep_cache import store_grep_result
...
def session_start_main() -> None:
    try:
        ...
        # later `cognikernel warm` / extraction backfills. recall / per-prompt injection
        # stay correct throughout via the deterministic lexical fallback.
        try:
            from cognikernel.embedding.model import is_ready, warm
            warm()
            if cwd and is_ready():
                from cognikernel.config import Config as _Cfg
                ...
                from cognikernel.storage.migrations import run_migrations as _rm
...
def _pretool_read(payload: dict) -> None:
    ...
    try:
        from cognikernel.config import Config
        from cognikernel.integration.lookup import decide_pretool_read
        from cognikernel.storage.connection import get_connection, get_db_path, resolve_project_id
        from cognikernel.storage.migrations import run_migrations
...
def _pretool_grep(payload: dict) -> None:
    ...
    try:
        from cognikernel.config import Config
        from cognikernel.storage.connection import get_connection, get_db_path, resolve_project_id
        from cognikernel.storage.grep_cache import lookup_grep_result
        from cognikernel.storage.migrations import run_migrations
...
def posttool_read_main() -> None:
    ...
    try:
        from cognikernel.config import Config
        from cognikernel.integration.lookup import resolve_post_read_outcome
...
def stop_main() -> None:
    ...
    if not session_id:
        _warn("cognikernel hook-stop: no session_id in payload — skipping extraction")
        return
    if not project_dir:
        _warn("cognikernel hook-stop: no cwd/project_dir in payload — skipping extraction")
        return
    ...
    if jsonl_path is None:
        _warn(f"cognikernel hook-stop: JSONL not found for session {session_id} — skipping")
        return
    ...
    try:
        ...
        if result.returncode != 0:
            _warn(f"cognikernel hook-stop: capture failed (rc={result.returncode}): {result.stderr[:300]}")
        else:
            _warn(f"cognikernel hook-stop: captured session {session_id} → {result.stdout.strip()[:200]}")
    except subprocess.TimeoutExpired:
        _warn("cognikernel hook-stop: capture timed out after 15s — jobs stay queued for the next drain")
    except Exception as exc:
        _warn(f"cognikernel hook-stop: unexpected error: {exc}")
    finally:
    ...
    try:
        ...
        _warn(f"cognikernel hook-stop: drain → {drain.stdout.strip()[:160]}")
    except subprocess.TimeoutExpired:
        _warn("cognikernel hook-stop: drain hit the 180s ceiling — remaining jobs stay queued")
    except Exception as exc:
        _warn(f"cognikernel hook-stop: drain error: {exc}")
