...
this module exists to prevent:
  ...
  1. Re-downloading every session. fastembed's default cache is the OS temp dir,
     ...
     pin a PERSISTENT cache under the cognikernel data dir so it downloads once, ever.
...
def _cache_dir() -> str:
    ...
    Honors COGNIKERNEL_DIR (tests/CI redirect the data dir there); otherwise ~/.cognikernel.
    """
    base = os.environ.get("COGNIKERNEL_DIR") or str(Path.home() / ".cognikernel")
    d = Path(base) / "models"
    try:
...
def warm() -> None:
    ...
    Never blocks. Suppressed by COGNIKERNEL_DISABLE_AUTO_WARM=1 (set by tests/CI in
    conftest) so the unit suite never triggers a background model download.
    """
    if os.environ.get("COGNIKERNEL_DISABLE_AUTO_WARM"):
        return
    _start_load()
...
def ensure_ready(timeout: float | None = None) -> bool:
    ...
    ask to wait: `cognikernel warm`, is_available(), real-model test gates. `timeout=None`
