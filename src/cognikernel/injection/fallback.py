...
Three distinct failure modes, three distinct messages:
  ...
  - Corrupted storage → tell user to run cognikernel doctor
"""
...
def render_or_fallback(ctx: Optional[InjectionContext]) -> str:
    ...
    from cognikernel.injection.template import render_with_budget_enforcement
...
def _fallback_uninitialized() -> str:
    return (
        ...
        "CogniKernel is not initialized for this project. "
        "Run `cognikernel init` in the project root to enable session memory."
    )
...
def _fallback_corrupted(ctx: InjectionContext) -> str:
    return (
        ...
        f"Run `cognikernel doctor` to diagnose. Continuing without context."
    )
