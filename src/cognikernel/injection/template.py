...
_log = logging.getLogger("cognikernel.injection")
...
def _render_header(ctx: InjectionContext) -> str:
    from cognikernel.injection.ckl import CKL_LEGEND
    header = (
    ...
    if ctx.ckl_mode:
        header += f"\n{CKL_LEGEND}"
    if ctx.ckl_v2:
        from cognikernel.injection.ckl import CKL_OPS_LEGEND
        header += f"\n{CKL_OPS_LEGEND}"
    return header
...
def _render_hard_constraints(
    ...
    if ckl_mode:
        from cognikernel.injection.ckl import render_event_ckl
